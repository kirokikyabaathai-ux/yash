/**
 * Admin Move Timeline Forward API Route
 * 
 * Allows admin to complete all steps up to and including a selected step.
 * 
...
export async function POST(
  ...
  try {
    ...
    const stepsToComplete = allSteps.filter((step) => {
      const stepOrderIndex = (step.step_master as any).order_index;
      return stepOrderIndex <= targetOrderIndex && step.status !== 'completed';
    });
    ...
    for (const step of stepsToComplete) {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          remarks: remarks || 'Admin override - moved timeline forward',
      ...
      await supabase.from('activity_log').insert({
        lead_id: leadId,
        user_id: user.id,
        action: 'admin_override_complete',
    ...
    const nextStep = allSteps.find((step) => {
      const stepOrderIndex = (step.step_master as any).order_index;
      return stepOrderIndex === targetOrderIndex + 1 && step.status === 'upcoming';
    });
    ...
    return NextResponse.json({
      success: true,
      message: `Moved timeline forward, completed ${stepsToComplete.length} step(s)`,
      completed_steps: stepsToComplete.length,
    });
  } catch (error) {
    console.error('Admin move forward error:', error);
    return NextResponse.json(
      {
        error: {
          code: 'INTERNAL_ERROR',
